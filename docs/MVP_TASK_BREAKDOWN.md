...
已完成 P0。当前已实现候选人、简历文件、字段来源三类数据表和 Alembic 迁移；已实现 PDF、DOCX、TXT 上传、文件类型校验、10MB 文件大小限制、本地 uploads 存储、文本抽取、结构化解析占位服务、低置信度字段、字段来源保存、预览文本接口和前端上传页。PDF 与 DOCX 已使用 `简历数据` 中的样例验证通过。老版 `.doc`、图片简历 OCR 和 ZIP 批量包不进入 MVP 当前范围。

说明：当前结构化解析使用本地规则解析作为 P0 占位，负责先跑通“上传、抽取、结构化保存、预览、字段来源”链路；后续如要接入国内大模型做更强的简历结构化解析，应在 AI Provider 层替换，不改变上传与数据模型边界。

AI Provider 状态：已新增 DeepSeek/OpenAI-compatible Provider，默认 `AI_BASE_URL=https://api.deepseek.com`，默认模型 `deepseek-v4-flash`，并保留 `deepseek-v4-pro` 配置选项。真实 key 放在被 Git 忽略的 `backend/.env`，`.env.example` 保持占位符。Provider 已用极小 JSON 请求验证连通，后续可接入简历结构化解析。
...
- [x] HR 可以上传 PDF、DOCX、TXT 文件。
- [x] 不支持的文件类型会被拦截并提示。
- [x] 上传成功后可以看到解析状态。
- [x] 解析失败会记录失败原因。
- [x] 解析成功后可以生成候选人基础信息、抽取文本、技能和字段来源。
...
已完成 P0。当前已实现字段修正日志表和 Alembic 迁移；已实现候选人修正数据接口、候选人字段更新接口和修改记录查询接口；前端已实现 `/jobs/:jobId/candidates/:candidateId/review` 修正页，支持左侧结构化字段编辑、右侧原简历抽取文本预览、低置信度字段提示和修改记录展示。接口已通过临时 FastAPI 服务验证：修正数据读取、候选人字段更新、修改日志写入和查询均可用。

P1 暂未实现：点击字段定位原文位置、原文片段高亮和关键词命中展示。
...
已完成。当前已实现 `candidate_matches` 数据表和 Alembic 迁移；已实现匹配评分服务、DeepSeek AI 评分调用、本地规则兜底、自动评分、手动重新评分、最新匹配结果查询接口；前端修正页已展示匹配分、推荐等级、评分摘要，并支持人工修正后点击“重新评分”。接口已通过样例简历验证：上传后自动生成评分，`GET /api/jobs/:jobId/candidates/:candidateId/match` 可读取最新评分，`POST /api/jobs/:jobId/candidates/:candidateId/match` 可重新评分。
...
- [x] 每个上传成功的候选人自动生成总匹配分。
- [x] 每个匹配结果都有推荐等级。
- [x] 每个匹配结果都有匹配理由。
- [x] 每个匹配结果可以展示短板和风险点。
- [x] 风险点以“待确认”或复核提示方式呈现。
- [x] 人工修正字段后可以重新生成匹配结果。
...
已完成。当前已实现 `candidate_job_statuses` 数据表和 Alembic 迁移；后端已实现候选人列表、候选人详情和候选人状态更新接口；前端已实现 `/jobs/:jobId/candidates` 候选人列表页和 `/jobs/:jobId/candidates/:candidateId` 候选人详情页。列表页支持按状态、推荐等级、最低分筛选，并按匹配分排序；详情页展示 AI 匹配分析、结构化信息和原简历预览，并提供编辑解析结果和重新评分入口。接口已通过样例简历验证：列表、详情、匹配结果展示和状态更新均可用。
...
- [x] 候选人可以按岗位查看。
- [x] 候选人可以按匹配分排序。
- [x] 候选人可以按推荐等级、状态、最低匹配分筛选。
- [x] HR 可以进入候选人详情。
- [x] 详情页可以看到 AI 分析、结构化信息和原简历。
- [x] HR 可以修改候选人状态。

### 3.7 Phase 6：MVP 收尾与验收

状态：已完成 Phase 6 收尾实现；端到端人工验收步骤详见 [MVP_ACCEPTANCE_GUIDE.md](./MVP_ACCEPTANCE_GUIDE.md)。
